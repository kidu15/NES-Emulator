...
uint8_t p6502::read(uint16_t a)
{
	return bus->read(a, false);
}

void p6502::write(uint16_t a, uint8_t b)
{
	bus->write(a, b); 
}

uint8_t p6502::GetFlag(FLAGS6502 f)
{
	return status;
}

void p6502::SetFlag(FLAGS6502 f, bool v)
{
	if (v)
		status |= f;
	else
...
void p6502::clock() 
{
	if (cycles == 0)
	{
		...
		//get starting number of cycles from the big table 
...
// implementation of the addressing modes 
...
uint8_t p6502::REL()
{
	addr_rel = read(pc);
	pc++;
	if (addr_rel & 0x80) //signed numbers contains the byte 7 to 1
		addr_rel |= 0xFF00;
	return 0;
}
